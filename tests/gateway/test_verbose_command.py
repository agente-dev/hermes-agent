...
class TestVerboseCommand:
    ...
    @pytest.mark.asyncio
    async def test_uses_platform_default_when_no_tool_progress_set(self, tmp_path, monkeypatch):
        """When tool_progress is not in config, use the platform default."""
        hermes_home = tmp_path / "hermes"
        hermes_home.mkdir()
        config_path = hermes_home / "config.yaml"
        config_path.write_text(
        ...
        # Telegram's built-in default is "new" -> cycles to all.
    ...
    async def test_per_platform_isolation(self, tmp_path, monkeypatch):
        ...
        default: Telegram = 'new' (concise high tier), Slack = 'off' (quiet Slack default).
        """
        hermes_home = tmp_path / "hermes"
        hermes_home.mkdir()
        config_path = hermes_home / "config.yaml"
        ...
        # Telegram: new -> all (concise high tier default = new)
