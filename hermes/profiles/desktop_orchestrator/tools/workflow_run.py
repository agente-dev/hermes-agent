    ...
    try:
        ...
        for step in steps:
            ...
            tid = kb.create_task(
                ...
                initial_status="todo",  # let the graph decide ready vs todo
...
def resume_step(task_id: str, decision: str, reason: Optional[str] = None, **kw: Any) -> str:
    ...
    try:
        with kb.connect_closing(board=board) as conn:
            # Best-effort: if the task is blocked, unblock it via the db helper
            # that the kanban_unblock tool also uses. Nudge afterwards.
            try:
                kb.unblock_task(conn, task_id=task_id, reason=reason or decision)
            except Exception:
                pass
        nudge(board=board)
