...
def _json_rpc_ok(rid: Any, result: Dict[str, Any]) -> Dict[str, Any]:
    return {"jsonrpc": "2.0", "id": rid, "result": result}


def _json_rpc_error(rid: Any, code: int, message: str) -> Dict[str, Any]:
    return {
        "jsonrpc": "2.0",
        "id": rid,
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _ensure_tool_registry_loaded(self):
    ...
    def _load_tui_rpc_server(self):
        ...
        if not getattr(tui_server, "_api_server_stdout_suppressed", False):
            tui_server.write_json = lambda _obj: True
            setattr(tui_server, "_api_server_stdout_suppressed", True)
        return tui_server

    def _dispatch_tui_rpc(self, request_body: Dict[str, Any]) -> Dict[str, Any]:
        tui_server = self._load_tui_rpc_server()
        ...
        if method == "auth.poll_subscription_oauth":
            rid = body.get("id")
            session_id = str(params.get("session_id") or "").strip()
            if not session_id:
                return _json_rpc_error(rid, 4002, "session_id required")
            try:
                tui_server._oauth_subscription_gc()
                with tui_server._oauth_subscription_lock:
                    state = dict(tui_server._oauth_subscription_sessions.get(session_id) or {})
                if not state:
                    return _json_rpc_error(
                        rid,
                        4040,
                        "subscription oauth session not found or expired",
                    )
                return _json_rpc_ok(
                    rid,
                    {
                        ...
                        "error_he": state.get("error_he"),
                    },
                )
            except Exception as exc:
                return _json_rpc_error(rid, 5004, str(exc))

        response = tui_server.handle_request(body)
    ...
    async def _handle_rpc(self, request: "web.Request") -> "web.Response":
        ...
        try:
            ...
            return web.json_response(_json_rpc_error(None, -32700, "parse error"))
        if not isinstance(body, dict):
            return web.json_response(_json_rpc_error(None, -32600, "invalid request"))

        rid = body.get("id")
        method = body.get("method")
        if not isinstance(method, str) or not method:
            return web.json_response(_json_rpc_error(rid, -32600, "method required"))
        params = body.get("params") or {}
        if not isinstance(params, dict):
            return web.json_response(_json_rpc_error(rid, -32602, "params must be an object"))

        if method == "approval.respond":
            session_key = (
                params.get("session_key")
                or params.get("sessionKey")
                or params.get("gateway_session_key")
                or params.get("session_id")
            )
            run_id = params.get("run_id")
            if not session_key and run_id:
                session_key = self._run_approval_sessions.get(str(run_id))
            if not session_key:
                return web.json_response(
                    _json_rpc_error(rid, 4002, "session_id or run_id required")
                )
            try:
                ...
                resolved = resolve_gateway_approval(
                    str(session_key),
                    choice,
                    resolve_all=bool(params.get("all") or params.get("resolve_all")),
                )
                if run_id:
                    self._set_run_status(str(run_id), "running", last_event="approval.responded")
                return web.json_response(_json_rpc_ok(rid, {"resolved": resolved}))
            except Exception as exc:
                return web.json_response(_json_rpc_error(rid, 5004, str(exc)))

        if method in {
            "auth.start_subscription_oauth",
            "auth.submit_oauth_code",
            "auth.poll_subscription_oauth",
            "clarify.respond",
            "sudo.respond",
            "secret.respond",
        }:
            return web.json_response(self._dispatch_tui_rpc(body))

        return web.json_response(_json_rpc_error(rid, -32601, f"unknown method: {method}"))
