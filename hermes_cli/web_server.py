...
def _write_session_token_file() -> None:
    """Atomic write + 0600 chmod of the current _SESSION_TOKEN to
    get_hermes_home()/session_token (sibling to other hermes_home artifacts).

    Follows the exact tempfile+replace+fsync+chmod( stat.S_IRUSR|stat.S_IWUSR )
    precedent from _save_anthropic_oauth_creds (around 2892-2911).

    Timing (per AC): inside start_server(), after token finalized (module-level
    env or fresh mint) + after bound_* stash, before uvicorn.run.
    This is the narrow "we are committing to serve" gate.

    - Rotation: overwrites on every start_server (fresh or env).
    - Idempotent/safe re-entry.
    - Path for user-facing: display_hermes_home()/session_token (never hardcode
      ~/.hermes in comments/docs).
    - data_access_mode local-only: the file is never sent over wire; desktop
      reads it from local FS only. The actual token value is NEVER logged,
      printed, or embedded in source/comments/error msgs.
    """
    token_path = get_hermes_home() / "session_token"
    token_path.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = token_path.with_name(
        f"{token_path.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        with tmp_path.open("w", encoding="utf-8") as handle:
            handle.write(_SESSION_TOKEN)
            handle.flush()
...
def _remove_session_token_file() -> None:
    """Best-effort cleanup of display_hermes_home()/session_token on shutdown.

    Registered via atexit from start_server. Must never raise or crash caller.
    """
    try:
...
def start_server(
    ...
    *,
    ...
    # Persist the (env or freshly-minted) _SESSION_TOKEN for local desktop
    # access (data_access_mode=local-only per hermes-202606-001). File:
    #   display_hermes_home()/session_token   (0600, atomic write+rename)
    # Must happen AFTER bound_* stash, BEFORE uvicorn.run (commit-to-serve gate).
    # Rotation on every start_server; best-effort atexit unlink on shutdown.
    # Uses the helper (which follows the oauth json atomic pattern exactly).
    # The token value itself is never logged/printed/surfaced.
    _write_session_token_file()
    atexit.register(_remove_session_token_file)
